...
* GetCollaboratorRole()
  Two overrides to get the parent collaboratorRole for the object.
...
{
    public partial class UserAuthorisation : Abstract
    {
        private SilkFlo.Data.Core.Domain.Business.UserAuthorisation _core;
        #region Constructors
        
        // This constructor is used by HTTP posts 
        public UserAuthorisation ()
        {
            _core = new SilkFlo.Data.Core.Domain.Business.UserAuthorisation();
        }
        public UserAuthorisation(SilkFlo.Data.Core.Domain.Business.UserAuthorisation core)
        {
            _core = core ??   
	        throw new NullReferenceException("The SilkFlo.Data.Core.Domain.Business.UserAuthorisation cannot be null");
        }
        #endregion

        #region Properties
        public string DisplayText { get; set; }
        public SilkFlo.Data.Core.Domain.Business.UserAuthorisation GetCore()
        {
            return _core;
        }
        public void SetCore(SilkFlo.Data.Core.Domain.Business.UserAuthorisation core)
        {
            _core = core;
        }
        ...
        {
            get => _core.CollaboratorRoleId;
            set
            {
                if (_core.CollaboratorRoleId == value)
                    return;

                _core.CollaboratorRoleId = value;
                _collaboratorRole = null;
            }
        }
        private Models.Business.CollaboratorRole _collaboratorRole;
        public Models.Business.CollaboratorRole CollaboratorRole
        {
            get
            {
                if (_collaboratorRole != null)
                    return _collaboratorRole;

                if (_core.CollaboratorRole != null)
                    _collaboratorRole = new Models.Business.CollaboratorRole(_core.CollaboratorRole);

                return _collaboratorRole;
            }
            set
            {
                if (_collaboratorRole == value)
                    return;

                _collaboratorRole = value;

                if (_collaboratorRole == null)
                    _core.CollaboratorRole = null;
                else
                {
                    if (_core.CollaboratorRoleId != _collaboratorRole.Id)
                        _core.CollaboratorRole = _collaboratorRole.GetCore();

                    _core.CollaboratorRoleId = _collaboratorRole.Id;
                }
            }
        }

        [DisplayName("Collaborator Role")]
        public string CollaboratorRoleString => CollaboratorRole?.ToString();

        public string CollaboratorRoleId_ErrorMessage { get; set; } = "Required";
        public bool CollaboratorRoleId_IsInValid { get; set; }
        ...
        {
            get => _collaboratorRoles ??= new List<Models.Business.CollaboratorRole>();
        ...
        ViewModels.Feedback feedback)
        {
            ...
            if (feedback.Elements.ContainsKey("CollaboratorRoleId"))
                feedback.Elements["CollaboratorRoleId"] = message;
            else
                feedback.Elements.Add("CollaboratorRoleId", message);
        ...
        public static List<Models.Business.UserAuthorisation> Create(IEnumerable<Data.Core.Domain.Business.UserAuthorisation> cores)
        {
            return cores.Select(core => new Models.Business.UserAuthorisation(core)).ToList();
        }
        
        public static Models.Business.UserAuthorisation[] Create(Data.Core.Domain.Business.UserAuthorisation[] cores)
        {
            return cores.Select(core => new Models.Business.UserAuthorisation(core)).ToArray();
        }
    }
}